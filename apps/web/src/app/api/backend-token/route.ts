...
export async function GET() {
  ...
  if (!rawPrivateKey) return NextResponse.json({ detail: 'Backend token signing is not configured.' }, { status: 503 });
  ...
  const token = await new SignJWT({ email: session.user.email ?? undefined, role: 'member' })
    ...
    .setSubject(session.user.id).setIssuer('accessforge-web').setAudience('accessforge-api')
    .setIssuedAt(now).setExpirationTime(now + 300).setJti(crypto.randomUUID()).sign(privateKey);
